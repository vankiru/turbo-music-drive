    ...
    <div>
          ...
          <div className="flex flex-col justify-center">
            <div className="flex items-center flex-col align-middle">
              ...
              {artist.current_listeners > 0 ? (
                <span className="text-red-500 text-small animate-pulse">
                  {artist.current_listeners} listening now
                </span>
              ) : (
      ...
      <div>
        ...
        <ul className="w-full border-b border-gray-200 flex text-plain mt-4">
          <li
            className={
              "inline-block px-4 text-plain border-b-2 border-white transition-colors" +
              (sort_tracks == "a-z" && "text-red-500")
            }
          >
            <Link
              href={api.artists.show.path({
                ...artist,
                query: { sort_tracks: "a-z" },
              })}
              className="hover:text-red-700 text-small pb-[2px]"
              preserveState
              preserveScroll
            >
              A→Z
          ...
          <li
            className={
              "inline-block px-4 text-plain border-b-2 border-white transition-colors" +
              (sort_tracks == "z-a" && "text-red-500")
            }
          >
            <Link
              href={api.artists.show.path({
                ...artist,
                query: { sort_tracks: "z-a" },
              })}
              className="hover:text-red-700 text-small pb-[2px]"
              preserveState
              preserveScroll
          ...
          <li
            className={
              "inline-block px-4 text-plain border-b-2 border-white transition-colors" +
              (sort_tracks == "old-new" && "text-red-500")
            }
          >
            <Link
              href={api.artists.show.path({
                ...artist,
                query: { sort_tracks: "new-old" },
              })}
              className="hover:text-red-700 text-small pb-[2px]"
              preserveState
              preserveScroll
          ...
          <li
            className={
              "inline-block px-4 text-plain border-b-2 border-white transition-colors" +
              (sort_tracks == "new-old" && "text-red-500")
            }
          >
            <Link
              href={api.artists.show.path({
                ...artist,
                query: { sort_tracks: "old-new" },
              })}
              className="hover:text-red-700 text-small pb-[2px]"
              preserveState
              preserveScroll
      ...
      <ul className="mt-2">
        {artist.tracks &&
          artist.tracks.map((track, counter) => (
