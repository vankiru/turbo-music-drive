...
export default function Show({ artist, analytics, albums, tracks, sort_tracks }) {
  return (
    <div>
      <Header/>
      ...
      <div className="py-4 flex justify-between items-center">
        ...
        <Analytics artist={artist} analytics={analytics} sort_tracks={sort_tracks}/>
      </div>
      ...
      <div>
        ...
        <ul className="w-full border-b border-gray-200 flex text-plain mt-4">
          <li className={"inline-block px-4 text-plain border-b-2 border-white transition-colors" + (sort_tracks == "a-z" && "text-red-500")}>
            <Link href={api.artists.show.path({...artist, query: {sort_tracks: 'a-z', period: analytics.period}})} className="hover:text-red-700 text-small pb-[2px]" preserveState preserveScroll>A→Z</Link>
          </li>
          <li className={"inline-block px-4 text-plain border-b-2 border-white transition-colors" + (sort_tracks == "z-a" && "text-red-500")}>
            <Link href={api.artists.show.path({...artist, query: {sort_tracks: 'z-a', period: analytics.period}})} className="hover:text-red-700 text-small pb-[2px]" preserveState preserveScroll>Z→Y</Link>
          </li>
          <li className={"inline-block px-4 text-plain border-b-2 border-white transition-colors" + (sort_tracks == "old-new" && "text-red-500")}>
            <Link href={api.artists.show.path({...artist, query: {sort_tracks: 'new-old', period: analytics.period}})} className="hover:text-red-700 text-small pb-[2px]" preserveState preserveScroll>Old→New</Link>
          </li>
          <li className={"inline-block px-4 text-plain border-b-2 border-white transition-colors" + (sort_tracks == "new-old" && "text-red-500")}>
            <Link href={api.artists.show.path({...artist, query: {sort_tracks: 'old-new', period: analytics.period}})} className="hover:text-red-700 text-small pb-[2px]" preserveState preserveScroll>New→Old</Link>
          </li>
        </ul>
      </div>
      <ul className="mt-2">
        {tracks.map((track, counter) => (<Track track={track} track_counter={counter} />))}
      </ul>
    </div>
  );
}
